...
namespace ChatWithSignalR
{
    ...
    public class ChatHub : Hub
    {
        ...
        public async Task SendMessage(string message, string receiverUserId)
        {
            ...
            var targetUserConnectionIdList = _connections.GetConnections(receiverUserId).ToList()[^1];
            await Clients.Client(targetUserConnectionIdList).SendAsync("ChatChannel", message);
